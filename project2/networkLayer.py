...
def main():
    ...
    while 1:
        try:
            ...
            n += 1
            print(time.asctime() +
                  " --- New connection ("+str(n)+") from "+addr[0])
            server = threading.Thread(target=networkLayer, name='%s' % (
                str(n)+str(addr)), args=[cx, addr, n])
            server.start()
...
def networkLayer(cx1, addr, n):
    ...
    r2 = threading.Thread(target=networkLayerReader, name='reader CX2 %s' % (
        str(n)+str(addr)), args=[cx2, cx1])
    r1 = threading.Thread(target=networkLayerReader, name='reader CX1 %s' % (
        str(n)+str(addr)), args=[cx1, cx2])
    w1 = threading.Thread(target=networkLayerWriter, name='writer CX1 %s' % (
        str(n)+str(addr)), args=[cx2, cx1])
    w2 = threading.Thread(target=networkLayerWriter, name='writer CX2 %s' % (
        str(n)+str(addr)), args=[cx1, cx2])
    r2.start()
    r1.start()
    w1.start()
    ...
    try:
        while True:
            ...
            if len(blocks[cxOther]) > MAXBLOCKS:
                ...
                print(
                    'len(blocks[cxOther]>MAXBLOCKS) occured from '+str(cxOther))
                cxOther.close()
                reader.close()
                return
    except Exception as e:
    ...
    try:
        while True:
            ...
            if l > 0:
                ...
                print("len(blocks[writer]) will be " +
                      str(len(blocks[writer]))+", after picking off this block.")
                ...
                if pd <= PROB_DEL:
                    ...
                    if pm <= PROB_MANGLING:
                        ...
                        # grab a random number of bytes
                        for i in range(1, random.randint(1, len(b))):
                            b = replaceChar(
                                b, i*random.randint(1, 1024) % len(b))
                    print(
                        "Writing block b[%d], of size len(b) =%d" % (r, len(b)))
